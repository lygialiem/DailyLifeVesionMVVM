...
class ReadingVC: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    readingCollectionView.scrollToItem(at: indexPath, at: .centeredHorizontally, animated: false)
  }
   ...
   print("INDEX PATH: ", indexPath)
    LibraryAPI.instance.getArticles(topic: self.concernedTitle ?? "", page: 4, numberOfArticles: 15) { (data) in
      DispatchQueue.main.async {
        ...
        let mustHaveImageData = uniqueData.filter({!($0.urlToImage == nil || ($0.urlToImage?.isEmpty) ?? Bool() )})
